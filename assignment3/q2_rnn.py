...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) >= max_length:
            sentence_mod = sentence[:max_length]
            labels_mod = labels[:max_length]
        else:
            sentence_mod = sentence + (max_length - len(sentence)) * [zero_vector]
            labels_mod = labels + (max_length - len(sentence)) * [zero_label]
            mask[len(sentence):] = [False] * (max_length - len(sentence))
        ret.append((sentence_mod, labels_mod, mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_loss_op(self, preds):
        ...
        masked_labels = tf.boolean_mask(self.labels_placeholder,
                                        self.mask_placeholder)
        loss = tf.reduce_mean(
            tf.nn.sparse_softmax_cross_entropy_with_logits(logits=masked_pred,
